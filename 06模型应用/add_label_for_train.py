...
classifier = fasttext.load_model("../model/best_classifier.model.bin", label_prefix="__label__")
end_load_time = datetime.datetime.now()
print('load model over,time is', (end_load_time - start_load_time).seconds, 's')

start_time = datetime.datetime.now()
with open('../data/seged_train_item.tsv', encoding='utf-8') as seged_test, \
        open('../data/train_label.txt', 'w') as label_test:
    cnt = 0
...
with open("../data/train_label.txt", encoding='utf-8') as f:
    test_label = [r.rstrip('\n') for r in f.readlines()]
result = itertools.zip_longest(test_item, test_label, fillvalue=' ')
with open("../data/train_added_label.tsv", "w", encoding='utf-8') as f:
    [f.write('\t'.join(r) + "\n") for r in result]
